...
void ANNbd_shrink::ann_search(ANNdist box_dist, kd_search_state& state)
{

	ANNdist inner_dist = 0;						// distance to inner box
	for (int i = 0; i < n_bnds; i++) {			// is query point in the box?
		if (bnds[i].out(state.ANNkdQ)) {				// outside this bounding side?
												// add to inner distance
			inner_dist = (ANNdist) ANN_SUM(inner_dist, bnds[i].dist(state.ANNkdQ));
		}
	}
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_search(inner_dist, state);	// search inner child first
		child[ANN_OUT]->ann_search(box_dist, state);	// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_search(box_dist, state);	// search outer child first
		child[ANN_IN]->ann_search(inner_dist, state);	// ...then outer child
	}
