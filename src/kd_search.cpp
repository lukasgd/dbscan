	...
	double				eps)			// the error bound
{
	...
	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		...
		nn_idx[i] = state.ANNkdPointMK.ith_smallest_info(i);
	}
}
...
void ANNkd_split::ann_search(ANNdist box_dist, kd_search_state& state)
{

										// distance to cutting plane
	ANNcoord cut_diff = state.ANNkdQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		child[ANN_LO]->ann_search(box_dist, state);// visit closer child first

		ANNcoord box_diff = cd_bnds[ANN_LO] - state.ANNkdQ[cut_dim];
		...
		if (box_dist * state.ANNkdMaxErr < state.ANNkdPointMK.max_key())
			child[ANN_HI]->ann_search(box_dist, state);

	}
	else {								// right of cutting plane
		child[ANN_HI]->ann_search(box_dist, state);// visit closer child first

		ANNcoord box_diff = state.ANNkdQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		box_dist = (ANNdist) ANN_SUM(box_dist,
				ANN_DIFF(ANN_POW(box_diff), ANN_POW(cut_diff)));
										...
										// visit further child if close enough
		if (box_dist * state.ANNkdMaxErr < state.ANNkdPointMK.max_key())
			child[ANN_LO]->ann_search(box_dist, state);
...
void ANNkd_leaf::ann_search(ANNdist box_dist, kd_search_state& state)
{
	...
	min_dist = state.ANNkdPointMK.max_key(); // k-th smallest distance so far

	for (int i = 0; i < n_pts; i++) {	// check points in bucket

		pp = state.ANNkdPts[bkt[i]];			// first coord of next data point
		qq = state.ANNkdQ;					// first coord of query point
		dist = 0;

		for(d = 0; d < state.ANNkdDim; d++) {
		...
		if (d >= state.ANNkdDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			state.ANNkdPointMK.insert(dist, bkt[i]);
			min_dist = state.ANNkdPointMK.max_key();
		}
	}
